...
class ApigenControllerAdviceTests {
    ...
    @Test
    void givenMediaType_whenBodyIsNotCorrect_thenReturnErrorWithCodeAndMessage() throws Exception {
        // given
        // when
        MockHttpServletResponse response = mvc.perform(
                get("/advice/media-type")
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.result.errors[0].code", is(1203)))
                .andExpect(jsonPath("$.result.errors[0].message", is("Format not supported, supported formats: '[text/xml]'")))
                .andExpect(jsonPath("$.result.errors[0].element").doesNotExist())
                .andReturn().getResponse();
