...
import static org.apiaddicts.apitools.apigen.archetypecore.core.persistence.filter.FilterOperation.AND;
import static org.apiaddicts.apitools.apigen.archetypecore.core.persistence.filter.FilterOperation.OR;
import static org.apiaddicts.apitools.apigen.archetypecore.core.persistence.functions.ApigenFunctionsMetadataBuilderInitializer.REGEXP_FUNCTION;
...
public class ApigenSearchExecutor {
	...
	public <E> ApigenSearchResult<E> search(List<String> select, List<String> exclude, List<String> orderBy, List<String> expand, Filter filter, Pagination pagination, boolean total, Class<E> clazz) { // NOSONAR
	...
	private List<Object> getMatchingIds(List<String> orderBy, List<String> expand, Filter filter, Pagination pagination, Class<?> clazz, CriteriaBuilder builder) {
		...
		List<Selection<?>> orderSelect = orderFields(allFields, root, joins);
	...
	private Predicate getSingleValueCondition(FilterOperation operation, Value rootValue, Expression<Comparable> expression, CriteriaBuilder builder) {
		...
		Comparable value = ApigenSearchExecutorTypeConverters.convert(attributeType, valueStr);
	...
	private Predicate getListValueCondition(FilterOperation operation, Value rootValue, Expression<Comparable> expression, CriteriaBuilder builder) {
		...
		List<Comparable> values = rootValue.getValues().stream()
				.map(v -> ApigenSearchExecutorTypeConverters.convert(attributeType, v)).collect(Collectors.toList());
