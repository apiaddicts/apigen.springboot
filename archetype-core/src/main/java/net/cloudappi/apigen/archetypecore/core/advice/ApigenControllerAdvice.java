package net.cloudappi.apigen.archetypecore.core.advice;

import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.databind.exc.InvalidFormatException;
import lombok.extern.slf4j.Slf4j;
import net.cloudappi.apigen.archetypecore.core.errors.ApigenErrorManager;
import net.cloudappi.apigen.archetypecore.core.errors.DefaultApigenError;
...
import net.cloudappi.apigen.archetypecore.exceptions.*;
...
public class ApigenControllerAdvice {
	...
	@ResponseStatus(HttpStatus.BAD_REQUEST)
	public ApiResponse mediaTypeNotSupported(HttpMediaTypeNotAcceptableException ex) {
		ApiError error = errorManager.getError(DefaultApigenError.UNSUPPORTED_FORMAT, null, ex.getSupportedMediaTypes());
		return new ApiResponse().withResultErrors(errors(error));
	}
	...
	private static class FieldNameTranslator {
		...
		private Map<String, String> getDictionary(Class clazz) {
			...
			for (Field field : clazz.getDeclaredFields()) {
				...
				if (pkg != null && !pkg.getName().startsWith("java") && !fieldClazz.equals(Filter.class)) {
